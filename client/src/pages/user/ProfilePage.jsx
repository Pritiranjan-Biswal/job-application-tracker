    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-400 mt-1">
          Manage your headline, technical skills stack, portfolio links, and security credentials
        </p>
      </div>

      {/* Profile Form */}
      <form onSubmit={handleSaveProfile} className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-2xl space-y-6">
          ...
          <div className="flex-1 w-full space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  disabled
                  value={user?.email || ''}
                  className="w-full px-3.5 py-2.5 text-sm bg-slate-900/80 border border-slate-800 rounded-xl text-slate-500 cursor-not-allowed outline-none font-mono"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 text-xs glass-input rounded-xl outline-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 pb-6 border-b border-slate-800">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Summary / Bio</label>
            <textarea
              rows={3}
              name="bio"
              value={profileData.bio}
              onChange={handleProfileChange}
              placeholder="Brief summary of your achievements, internships, and core engineering interests..."
        ...
        <div className="pb-6 border-b border-slate-800">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {profileData.skills.map((skill) => (
              <span
                key={skill}
                className="inline-flex items-center gap-1.5 px-3.5 py-1 bg-indigo-500/10 text-indigo-300 border border-indigo-500/30 rounded-xl text-xs font-mono font-bold"
              >
                {skill}
                <button
                  type="button"
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Add skill (e.g. Docker, TypeScript, Next.js, Redis)..."
              className="px-3.5 py-2 text-xs glass-input rounded-xl outline-none flex-1"
            />
            <button
              type="button"
              onClick={handleAddSkill}
              className="px-5 py-2 text-xs font-bold bg-slate-800 text-slate-200 hover:bg-slate-700 hover:text-white rounded-xl transition-colors"
            >
              Add Skill
            </button>
          </div>
        ...
        <div className="flex justify-end">
          <button
            ...
            className="inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-600/30 transition-all disabled:opacity-50 hover:scale-105"
          >
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-2xl">
        ...
        <p className="text-xs text-slate-400 mb-6">Ensure your account is protected with a strong password</p>
        ...
        <form onSubmit={handleUpdatePassword} className="space-y-4 max-w-md">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs rounded-xl transition-all disabled:opacity-50"
          >
