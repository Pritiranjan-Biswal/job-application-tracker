    ...
    <Modal isOpen={isOpen} onClose={onClose} title={title} maxWidth="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          {/* Salary / CTC */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Salary / CTC</label>
            <input
              type="text"
              name="salary"
              value={formData.salary}
              onChange={handleChange}
              placeholder="e.g. ₹18 LPA or $90k"
              className="w-full px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 outline-none"
            />
          </div>
        ...
        {/* Notes */}
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Preparation Notes / Referral info</label>
          <textarea
            name="notes"
            rows={3}
            value={formData.notes}
            onChange={handleChange}
            placeholder="e.g. Referred by senior. Key requirements: React, Redux, Node.js, DSA graph algorithms..."
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
          ...
          <button
            type="submit"
            disabled={loading}
            className="inline-flex items-center gap-2 px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm shadow-indigo-500/20 transition-all disabled:opacity-60"
          >
